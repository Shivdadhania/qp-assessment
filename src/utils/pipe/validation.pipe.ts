...
export class ValidationPipe implements PipeTransform<any> {
  ...
  async transform(value: any, { metatype }: ArgumentMetadata) {
    ...
    if (errors.length > 0) {
      const ero = errors[0] ?? { constraints: undefined, children: undefined };

      // Managed 1 level child request.
      const constraints = ero.constraints
        ? ero.constraints
        : ero.children[0].constraints;

      let firstErrorMessage = 'invalid field.';

      try {
        firstErrorMessage =
          constraints[Object.keys(constraints)[0]] ?? firstErrorMessage;
      } catch (e) {}
      throw new BadRequestException({
